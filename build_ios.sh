...
if ! find build/ios -name '*.pck' -print -exec sh -c 'strings "$1" | grep -q scripts/autoload/build_diagnostics.gdc' sh {} \;; then
	echo "Exported iOS project is missing build_diagnostics.gdc in its PCK"
	exit 1
fi
ls -la build/ios/
