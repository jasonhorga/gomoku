...
- ✅ **引入 VCF search（Victory by Continuous Four）** —— 代码里有 `vcf_search.py` 但没深度集成。强棋型 AI 的关键能力
...
ai_server/
  ai/
    ...
    pattern_eval.py     — 棋型分数（five/open_four/...）
    mcts_engine.py      — Hybrid MCTS（三种模式）
    vcf_search.py       — 战术搜索（未充分利用）
  nn/
